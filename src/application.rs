...
mod imp {
    ...
    impl ApplicationImpl for RobotsApplication {
        fn startup(&self) {
            ...
            glib::set_application_name(&g_("Robots"));
            ...
            match GameConfigs::load() {
                Ok(game_configs) => {
                    self.game_configs.set(game_configs);
                }
                Err(e) => {
                    eprintln!("CRITICAL {}\n{}",
                        g_("The program Robots was unable to find any valid game configuration files. Please check that the program is installed correctly."),
                        e
                    );
                    self.obj().quit();
                }
    ...
    impl RobotsApplication {
        fn new_game_cb(&self) {
            if let Some(window) = self.obj().active_window().and_downcast::<RobotsWindow>() {
                let dialog = adw::MessageDialog::new(
                    Some(&window),
                    Some(&g_("_New Game")),
                    Some(&g_("Are you sure you want to discard the current game?")),
                );

                dialog.add_response("cancel", &g_("Keep _Playing"));
                dialog.add_response("new", &g_("_New Game"));
        ...
        fn about_cb(&self) {
            let window = adw::AboutWindow::builder()
                ...
                .translator_credits(&g_("translator-credits"))
