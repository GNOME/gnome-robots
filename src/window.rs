...
use crate::controls::{key_from_i32, key_from_u32};
...
use crate::scores::scores::{add_score, show_scores, Category};
...
mod imp {
    ...
    impl ObjectSubclass for RobotsWindow {
        ...
        fn new() -> Self {
            Self {
                window_title: adw::WindowTitle::builder().title(g_("Robots")).build(),
        ...
        fn class_init(klass: &mut Self::Class) {
            klass.install_action("random-teleport", None, |obj, _, _| {
                obj.imp().random_teleport_cb();
            });
            klass.install_action("safe-teleport", None, |obj, _, _| {
                obj.imp().safe_teleport_cb();
            });
            klass.install_action("wait", None, |obj, _, _| {
                obj.imp().wait_cb();
            });
        }
    }
    ...
    impl RobotsWindow {
        fn random_teleport_cb(&self) {
            self.game_area
                .get()
                .unwrap()
                .player_command(PlayerCommand::RANDOM_TELEPORT);
        }
        ...
        fn safe_teleport_cb(&self) {
            self.game_area
                .get()
                .unwrap()
                .player_command(PlayerCommand::SAFE_TELEPORT);
        }
...
impl RobotsWindow {
    pub fn new(
        ...
        this.imp().settings.set(settings.clone());
        ...
        appmenu.append_section(None, &{
            let section = gio::Menu::new();
            section.append_item(&gio::MenuItem::new(
                Some(&g_("_Preferences")),
                Some("app.preferences"),
            ));
            section.append_item(&gio::MenuItem::new(Some(&g_("_Help")), Some("app.help")));
            section.append_item(&gio::MenuItem::new(
                Some(&g_("_About Robots")),
                Some("app.about"),
            ));
            section
        });
    ...
    fn update_game_status(&self, game: &Game) {
        ...
        let remaining_teleports_text =
            g_("(Remaining: %d)").replacen("%d", &status.safe_teleports.to_string(), 1);
        self.imp()
