...
impl GameArea {
    pub fn new(
        ...
        this.imp().game_configs.set(game_configs);
        this.imp().assets.set(assets.clone());
        *this.imp().background_color.borrow_mut() = settings.bgcolour();
    ...
    fn mouse_cb(&self, n_press: i32, x: f64, y: f64) {
        let game_ref = self.imp().game.borrow();
        let Some(game) = game_ref.as_ref() else {
            return;
        };
        if game.state() == State::PLAYING {
            let d = self.get_dir(game, x, y);
            let cmd = PlayerCommand::from_direction(d.0, d.1);
    ...
    fn get_dir(&self, game: &Game, ix: f64, iy: f64) -> (i32, i32) {
        ...
        let idx = (x as i32) - (player.x as i32);
        let idy = (y as i32) - (player.y as i32);
        if idx.abs() < 2 && idy.abs() < 2 {
            return (idx, idy);
        }

        // Otherwise go in the general direction of the mouse click.
        let dx = ix - (player.x as f64 + 0.5) * tile_width;
        let dy = iy - (player.y as f64 + 0.5) * tile_height;
        ...
        let octant = (((8.0 * angle / PI).floor() as usize + 9) / 2) % 8;
    ...
    fn on_game_event(&self, event: GameEvent) {
        match event {
            ...
            GameEvent::LEVEL_COMPLETE => self.play_sound(Sound::YAHOO),
            ...
            GameEvent::NO_TELEPORT_LOCATIONS => {
                self.message_box(&g_("There are no teleport locations left!!"));
            }
            _ => {}
        }
    }
