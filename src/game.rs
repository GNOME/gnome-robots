...
impl Game {
    ...
    fn add_kill(&self, _type: ObjectType) {
        let si = match (self.state.get(), _type) {
            (State::WAITING | State::WAITING_TYPE2, ObjectType::ROBOT1) => {
                self.kills.set(self.kills.get() + 1);
                self.config.borrow().score_type1_waiting
            }
            (State::WAITING | State::WAITING_TYPE2, _) => {
        ...
        *self.arena.borrow_mut() = change.arena;
        ...
        if self.arena.borrow().get(change.player) != ObjectType::PLAYER {
                ...
                <= 0
            {
                ...
                self.on_game_event.emit(GameEvent::LEVEL_COMPLETE);
                self.endlev_counter.set(0);
            }
        }
    }
    ...
    fn move_robots(&self) {
        ...
        if num_robots2 > 0 {
            if self.state.get() == State::WAITING {
                self.state.set(State::WAITING_TYPE2);
    ...
    fn random_teleport(&self) -> bool {
        let Some(p) = self
            ...
            self.on_game_event.emit(GameEvent::NO_TELEPORT_LOCATIONS);
            return false;
        };
    ...
    fn safe_teleport(&self) -> bool {
        ...
        let change = loop {
            let Some(p) = self
                ...
                self.on_game_event
                    .emit(GameEvent::NO_SAFE_TELEPORT_LOCATIONS);
                self.kill_player();
                return false;
            };
    ...
    pub fn player_command(
        ...
        match cmd {
            ...
            PlayerCommand::SAFE_TELEPORT => {
                if self.safe_teleport() {
                    self.move_robots();
                }
                true
            }
            PlayerCommand::RANDOM_TELEPORT => {
